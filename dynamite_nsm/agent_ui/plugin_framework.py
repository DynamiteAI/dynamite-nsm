...
class Plugin:
    ...
    def can_current_user_load(self):
        """
        Determines whether the current user has the correct permissions to load this plugin

        :return: True if the plugin can be loaded with current user's role
        """
        role = 'analyst'
        if current_user.has_role("admin"):
            ...
            role = "analyst"
        if self.permissions_require == 'admin':
            return role == "admin"
        elif self.permissions_require == 'superuser':
            return role in ['admin', 'superuser']
        elif self.permissions_require == 'analyst':
            return role in ['admin', 'superuser', 'analyst']
        return False
