...
class RemoveTargeteButton(npyscreen.ButtonPress):
    ...
    def whenPressed(self):
        kafka_enabled = self.parent.parentApp.filebeat_config.kafka_targets.enabled
        if kafka_enabled:
            target_type = 'Kafka'
        else:
        ...
        if kafka_enabled:
            kafka_config = self.parent.parentApp.filebeat_config.kafka_targets
            kafka_config.target_strings.remove(self.delete_value)
        else:
            logstash_config = self.parent.parentApp.filebeat_config.logstash_targets
            logstash_config.target_strings.remove(self.delete_value)

        self.parent.parentApp.filebeat_config.commit()
...
class ToggleKafkaOrLogstashTargetOutputsButton(npyscreen.ButtonPress):
    def whenPressed(self):
        ...
        if self.parent.parentApp.filebeat_config.kafka_targets.enabled:
            self.parent.parentApp.filebeat_config.switch_to_logstash_target()
            output_type = 'LogStash'
        else:
            self.parent.parentApp.filebeat_config.switch_to_kafka_target()
            output_type = 'Kafka'
...
class FilebeatInstanceSettingsForm(npyscreen.ActionForm):
    ...
    def beforeEditing(self):
        self.agent_tag.value = self.parentApp.filebeat_config.field_processors.originating_agent_tag

    def create(self):
        is_kafka_enabled = self.parentApp.filebeat_config.kafka_targets.enabled
        ...
        self.nextrely += 2
        ls_target_names = self.parentApp.filebeat_config.logstash_targets.target_strings
        ls_target_names = list(set(ls_target_names))
        ls_target_names.append('<create new target>')

        kf_target_names = self.parentApp.filebeat_config.kafka_targets.target_strings
    ...
    def on_ok(self):
        try:
            self.parentApp.filebeat_config.field_processors.originating_agent_tag = \
                self.agent_tag.value
            self.parentApp.filebeat_config.commit()
...
class EditTargetsForm(npyscreen.ActionForm):
    ...
    def create(self):
        ...
        if self.parentApp.filebeat_config.kafka_targets.enabled:
    ...
    def beforeEditing(self):
        kafka_enabled = self.parentApp.filebeat_config.kafka_targets.enabled
        if self.value == '<create new target>':
            self.value = None
        if self.value:
            self.target_text.value = self.value
            if kafka_enabled:
                kafka_config = self.parentApp.filebeat_config.kafka_targets
                self.topic_text.value = kafka_config.topic
                if kafka_config.username:
                    self.username_text.value = kafka_config.username
                if kafka_config.password:
                    self.password_text.value = kafka_config.password
    ...
    def on_ok(self):
        kafka_enabled = self.parentApp.filebeat_config.kafka_targets.enabled
        if not InstallManager.validate_targets(target_strings=[self.target_text.value]):
        ...
        if kafka_enabled:
            kafka_config = self.parentApp.filebeat_config.kafka_targets
            original_targets = list(kafka_config.target_strings)
        else:
            logstash_config = self.parentApp.filebeat_config.logstash_targets
            original_targets = list(logstash_config.target_strings)
        if self.value:
        ...
        if kafka_enabled:
            ...
            if self.password_text.value.strip() == '':
                password = None
            self.parentApp.filebeat_config.kafka_targets = targets.KafkaTargets(target_strings=original_targets,
                                                                                topic=self.topic_text.value,
                                                                                username=username, password=password,
                                                                                enabled=True)
        else:
            self.parentApp.filebeat_config.logstash_targets.target_strings = original_targets
