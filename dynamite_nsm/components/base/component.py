...
class BaseComponent:
    ...
    def __init__(self, component_name, component_description):
        self.component_name = component_name
        self.component_description = component_description

        self.install_strategy = exec_strategy.BaseExecStrategy()
        self.uninstall_strategy = exec_strategy.BaseExecStrategy()
        self.process_start_strategy = exec_strategy.BaseExecStrategy()
        self.process_stop_strategy = exec_strategy.BaseExecStrategy()
        self.process_status_strategy = exec_strategy.BaseExecStrategy()

    @staticmethod
    def validate_strategy(strategy):
        if not isinstance(strategy, exec_strategy.BaseExecStrategy):
            raise TypeError("Invalid strategy, must be {}.".format(type(exec_strategy.BaseExecStrategy)))
